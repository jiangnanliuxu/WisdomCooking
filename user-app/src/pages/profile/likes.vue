...
import { listMyLikes } from '@/api/user'
...
async function loadData() {
  ...
  const response = await listMyLikes({ targetType: targetType.value, page: '1', pageSize: '20' })
...
<template>
  <view class="page-body like-page">
    <view class="page-nav">
      <view class="page-nav__back" @click="goBack">‹</view>
      <view class="page-nav__title">我的点赞</view>
        ...
        <view v-for="(item, index) in displayRows" :key="item.targetId || index" class="surface-card recipe-card" @click="openItem(item)">
          ...
          <view class="recipe-card__info">
            ...
            <view class="recipe-card__bottom">
              ...
              <text class="recipe-card__liked">❤️ 已点赞</text>
            </view>
          </view>
        </view>
      </template>
        ...
        <view v-for="(item, index) in displayRows" :key="item.targetId || index" class="surface-card feed-card" @click="openItem(item)">
          <view class="feed-card__header">
            ...
            <view>
              ...
              <view class="feed-card__time">已点赞</view>
            </view>
          </view>
          ...
          <view class="feed-card__footer">
            <text class="feed-card__liked">❤️ 已点赞</text>
            <text class="feed-card__meta">💬 12</text>
          </view>
        </view>
      </template>
    </view>

    <EmptyState v-else title="还没有点赞内容" description="给喜欢的菜谱或动态点个赞，稍后会出现在这里。" />
  </view>
</template>
